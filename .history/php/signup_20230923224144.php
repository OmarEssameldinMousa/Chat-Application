            ...
            if (isset($_FILES['image'])) {
                ...
                if (in_array($img_ext, $extensions) === true) {
                    ...
                    if (move_uploaded_file($tmp_name, "../images/" . $new_img_name)) {
                        ...
                        $sql2 = mysqli_query($conn, "INSERT INTO users (user_id,unique_id, fname,lname,email,password,img,status)
                                                VALUES ($++user_id,{$random_id},'{$fname}','{$lname}','{$email}','{$password}','{$new_img_name}','{$status}')");
                        if ($sql2) {
